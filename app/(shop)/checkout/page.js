...
import {
  loadRazorpayScript, createRazorpayOrder,
  openRazorpayCheckout, verifyRazorpayPayment,
} from '@/lib/razorpay'
...
export default function CheckoutPage() {
  ...
  // ── Coupon — auto-loaded from persisted cart store ─────────────────────
  const {
  ...
  const { shippingCharge, shippingFree, codCharge } = getShipping(
    safeCartTotal, paymentMethod, items
  )
  ...
  const total = Math.max(0, safeCartTotal - discount + effectiveShippingCharge + codCharge)
  ...
  useEffect(() => {
    if (!authLoading && items.length === 0) router.push('/cart')
  ...
  async function handlePlaceOrder() {
    ...
    const baseOrderData = {
      ...
      taxBreakdown,
      shipping:        effectiveShippingCharge,
      codFee:          codCharge,
      discount,
      ...
      if (paymentMethod === 'cod') {
        ...
        Promise.allSettled([
          triggerEmail('order_confirmed', user.email, {
            order: { ...baseOrderData, status: 'confirmed' }, orderId,
          }),
          triggerEmail('admin_new_order', process.env.NEXT_PUBLIC_ADMIN_EMAIL, {
            order: baseOrderData, orderId,
          }),
        ])
      ...
      <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <form onSubmit={handleSaveNewAddress} className="space-y-4 border-t border-gray-100 pt-5">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input name="fullName" value={addressForm.fullName} onChange={handleAddressFormChange}
                        ...
                        <div>
                          ...
                          <input name="phone" value={addressForm.phone} onChange={handleAddressFormChange}
                      ...
                      <div>
                        ...
                        <input name="line1" value={addressForm.line1} onChange={handleAddressFormChange}
                          placeholder="House no, building, street"
                          className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">Address line 2</label>
                        <input name="line2" value={addressForm.line2} onChange={handleAddressFormChange}
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          <select name="state" value={addressForm.state} onChange={handleAddressFormChange}
                        ...
                        <div>
                          ...
                          <input name="pincode" value={addressForm.pincode} onChange={handleAddressFormChange}
                            maxLength={6}
                            className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                        </div>
                      </div>
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-6">
              ...
              <div className="space-y-3 mb-6">
                ...
                <label className={`flex items-start gap-3 border-2 rounded-xl p-4 cursor-pointer transition-colors ${
                  paymentMethod === 'razorpay' ? 'border-indigo-500 bg-indigo-50/50' : 'border-gray-200 hover:border-gray-300'
                }`}>
                  <input type="radio" name="paymentMethod" checked={paymentMethod === 'razorpay'}
                    onChange={() => setPaymentMethod('razorpay')} className="mt-1 accent-indigo-600" />
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      {['UPI','Cards','NetBanking','Wallets'].map((m) => (
                        <span key={m} className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded">{m}</span>
                      ))}
                    </div>
                  </div>
                </label>
                ...
                {!shippingLoading && shippingConfig.codEnabled && (
                  <label className={`flex items-start gap-3 border-2 rounded-xl p-4 cursor-pointer transition-colors ${
                    paymentMethod === 'cod' ? 'border-indigo-500 bg-indigo-50/50' : 'border-gray-200 hover:border-gray-300'
                  }`}>
                    <input type="radio" name="paymentMethod" checked={paymentMethod === 'cod'}
                      onChange={() => setPaymentMethod('cod')} className="mt-1 accent-indigo-600" />
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-6">
              ...
              <div className="border border-gray-100 rounded-xl p-4 mb-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={() => setStep(1)} className="text-xs text-indigo-600 font-medium hover:underline">
                    Change
                  </button>
                </div>
              ...
              <div className="border border-gray-100 rounded-xl p-4 mb-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={() => setStep(2)} className="text-xs text-indigo-600 font-medium hover:underline">
                    Change
                  </button>
                </div>
              ...
              <div className="border border-gray-100 rounded-xl p-4 mb-4">
                ...
                <div className="space-y-2">
                  {items.map((item) => (
                    <div key={`${item.productId}__${item.variantId || 'base'}`}
              ...
              {/* Coupon input — pre-filled from store */}
              <div className="mb-5">
                <CouponInput
                  code={couponCode}
        ...
        <div className="lg:col-span-1">
          <div className="bg-white border border-gray-100 rounded-2xl p-6 sticky top-24">
            ...
            <div className="space-y-3 mb-5">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-900">
                  {formatPrice(totalTaxAmount > 0 ? subtotalBeforeTax : safeCartTotal)}
                </span>
              </div>
            ...
            {totalTaxAmount > 0 && (
              <p className="text-xs text-gray-400 mt-2">Incl. {formatPrice(totalTaxAmount)} GST</p>
            )}
