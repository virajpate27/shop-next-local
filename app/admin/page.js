...
export default function AdminDashboard() {
  ...
  async function migrateOrderTimelines() {
  ...
  toast.success(`Migrated ${count} orders with timeline`)
}
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
            ...
            <button
              onClick={async () => {
                const res = await fetch(
                  `/api/email/low-stock?secret=${process.env.NEXT_PUBLIC_CRON_SECRET}`,
                );
                const data = await res.json();
                if (data.success)
                  toast.success(
                    `Stock alert sent for ${data.alerted} products`,
                  );
        ...
        <button
          onClick={migrateOrderTimelines}
          className="text-xs text-indigo-600 font-medium hover:underline"
        >
          Migrate order timelines
        </button>
      </div>
    </div>
  );
